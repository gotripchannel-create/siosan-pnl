...
- ЛЮБОЙ комментарий, где отдельным словом встречается «зп» (например «зп орхан», «рома зп», «зп курьер», просто «зп») — это зарплатная выплата конкретному сотруднику. Зарплата уже учитывается отдельно через ФОТ/смены сотрудников — НИКОГДА не добавляй такую строку в kitchenExpenses или otherExpenses (и не выдумывай для неё категорию вида «Зарплата»), просто полностью игнорируй её, как «дб». Единственное исключение — если это явно оплата именно курьеру («зп курьер», «курьер» с суммой) — тогда это идёт в courier.pay, а не в otherExpenses.
...
Если это ТЕКСТ ИЗ ВК — он может быть одним чистым сообщением с отчётом за один день, ИЛИ целиком скопированным куском истории чата за несколько дней, где вперемешку идут реальные отчёты, обычная переписка, приветствия, вопросы сотрудников, имена отправителей сообщений и т.п. Твоя задача — найти ВСЕ настоящие финансовые отчёты в тексте и вернуть их списком через submit_parsed_reports. Признак настоящего отчёта — ЛИБО явно посчитанная выручка (есть «наличные», «карта/карты» и «итого выручка» с числами), ЛИБО текст начинается со строки вида «Расходы за YYYY-MM-DD:» — это автоматически сформированный список расходов (изъятий наличных из кассы) БЕЗ выручки, извлечённый из iiko напрямую, а не из переписки. Такой блок ВСЕГДА нужно вернуть как отдельный отчёт с этой датой и заполненными kitchenExpenses/otherExpenses, даже если revenue у него пустой — это ожидаемо и нормально, не пропускай его и не считай нераспознанным. Всё остальное — приветствия, вопросы не про деньги, имена отправителей сообщений, реакции — полностью игнорируй, НЕ добавляй в unmatchedLines (unmatchedLines — только для строк ВНУТРИ найденного отчёта, которые ты не смог классифицировать).
