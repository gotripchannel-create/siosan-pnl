...
export default async function handler(req, res) {
  ...
  try {
    ...
    const expenses = rows
      ...
      .filter((e) => e.amount > 0 && e.date && e.comment !== 'дб' && e.comment !== 'зп' && e.comment !== 'бк' && !e.comment.startsWith('закрытие кассовой смены')) // "дб" — не расход, "зп" — уже учтена в ФОТ отдельно, "бк" — перенос остатка между сменами, "закрытие кассовой смены" — системная запись
