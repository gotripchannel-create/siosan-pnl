          ...
          <div style={{marginTop:16}}>
            ...
            {dayReport.revenue?.payIncomeAdded > 0 && (
              <div className="rp-cash-check" style={{marginTop:12}}>
                <Info size={13}/> В выручку дня включено внесение по заказу: <b>{fmtRub(dayReport.revenue.payIncomeAdded)}</b> — деньги, принятые отдельной кассовой операцией «Внесение наличных» (любой комментарий, кроме «дб» — начального остатка кассы), не проходят через обычную продажу и не попадают в отчёт по продажам, поэтому добавлены отдельно.
              </div>
            )}
      ...
      {!loading && !error && data && (
        <>
          ...
          {data.cashPayIncome > 0 && (
            <div className="rp-cash-check" style={{marginBottom:16}}>
              <Info size={13}/> В выручку за месяц включены внесения наличных: <b>{fmtRub(data.cashPayIncome)}</b> — деньги, принятые отдельной кассовой операцией «Внесение наличных» (любой комментарий, кроме «дб» — начального остатка кассы), не проходят через обычную продажу и не попадают в OLAP-отчёт, поэтому добавлены отдельно.
            </div>
          )}
